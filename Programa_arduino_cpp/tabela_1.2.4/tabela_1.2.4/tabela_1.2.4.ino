...
int elementoAtual = 0; // Variável para armazenar o elemento atual
...
void loop() {
    ...
    if (currentMillis - previousMillis >= interval) {
        ...
        // Ações para os botões Next e Previous
        if (estadoBotaoNext == LOW) {
            myDFPlayer.enableLoopAll(); // habilita o loop automático
            lastActivityMillis = currentMillis; // Atualiza o tempo da última atividade
        } else if (estadoBotaoPrev == LOW) {
            myDFPlayer.disableLoopAll(); // reproduz faixa aleatória
            lastActivityMillis = currentMillis; // Atualiza o tempo da última atividade
        }
